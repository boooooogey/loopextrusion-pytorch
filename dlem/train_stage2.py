...
import wandb
import dlem
from dlem.trainer_non_loop import LitTrainerStage2
from dlem.trainer import LitTrainer
from dlem.trainer_data import LitTrainerData

from dlem.non_loopextrusion_factor_model import NonLoopExt
...
parser.add_argument('--loop-model', type=str, required=True, help='Path to the loop model')
...
parser.add_argument('--project-name', type=str, default="DLEM_runs_stage2", help='Project name for wandb.')
...
LOOP_MODEL_PATH = args.loop_model
...
model_loop_trained = LitTrainer.load_from_checkpoint(checkpoint_path=LOOP_MODEL_PATH,
                                        model=model,
                                        loss=weighted_mse if LOSS_TYPE == 'weighted_mse' else torch.nn.MSELoss()).model

model_non_loop = NonLoopExt(rank_k=10, in_features=data_train.track_dim, patch_size=PATCH_SIZE)

model_training = LitTrainerStage2(model,
                            model_non_loop,
                            LEARNING_RATE,
