...
def _dlem(left, right, index_diag, n, curr_diag, const):
...
class DLEM(Module):
    ...
    def forward(self,
        ...
        left_right = self.converter(signal.to(dev))
        ...
        for index_diag in range(self.start_diag, self.stop_diag-1):
            next_diag_pred = _dlem(left,
                                   right,
                                   index_diag,
                                   self.n,
                                   torch.exp(diagonals[:, self.indexing(index_diag)]).to(dev),
                                   self.const)
            next_diag_pred = torch.log(next_diag_pred)
            next_diag_pred -= torch.mean(next_diag_pred)
            out[:, self.indexing_out(index_diag+1)] = next_diag_pred.cpu()
