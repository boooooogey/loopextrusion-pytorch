...
import torch
from torch.nn import Module, Parameter, Sequential, ReLU, Conv1d, ConvTranspose1d
from torch.nn import ModuleList, Sigmoid
from .. import util

class Crop(Module):
    """Crops the input from both sides in the last dimension.
    """
    def __init__(self, crop_size=1):
        super(Crop, self).__init__()
        self.crop_size = crop_size
    def forward(self, a:ArrayLike) -> ArrayLike:
        """Crops the input array

        Args:
            a (ArrayLike): to be cropped.

        Returns:
            ArrayLike: cropped array.
        """
        return a[...,self.crop_size:-self.crop_size]
...
class DLEM(Module):
    ...
    def __init__(self, n:int,
        ...
        self.diff_layer_num = diff_layer_num
        self.channel_per_route = channel_per_route
        network_width = layer_num * channel_per_route

        self.convs  = [conv_unit_maker(dim_num, network_width)]
        ...
        self.diffusion = Sequential(ConvTranspose1d(in_channels=2,
                                        out_channels=10,
                                        groups=2,
                                        kernel_size=3),
                                     ReLU(),
                                     Crop(),
                                    *[ConvTranspose1d(in_channels=10,
                                        out_channels=10,
                                        groups=10,
                                        kernel_size=3),
                                      ReLU(),
    ...
    def converter(self, signal:ArrayLike) -> ArrayLike:
        ...
        out = self.mixer(torch.cat(layer_outs, axis=1))
        return self.diffusion(out)
        #return torch.nn.functional.sigmoid(out + self.diffusion(out))
