...
class DLEM(torch.nn.Module):
    def __init__(self):
        ...
        self.num_channel = 25
        self.conv = torch.nn.Sequential(torch.nn.Conv1d(in_channels=2,
                                                        out_channels=self.num_channel,
                                                        kernel_size=3,
                                                        padding=1),
                                        torch.nn.ReLU(),
                                       *[torch.nn.Conv1d(in_channels=self.num_channel,
                                                         out_channels=self.num_channel,
                                                         kernel_size=3,
                                                         padding=1),
                                         torch.nn.ReLU()]*2,
                                       *[torch.nn.ConvTranspose1d(in_channels=self.num_channel,
                                                                  out_channels=self.num_channel,
                                                                  kernel_size=3,
                                                                  padding=1,
                                                                  output_padding=0),
    ...
    def forward(self, x, start, stop, transform=True):
        lr = torch.cat([x.sum(axis=-2), x.sum(axis=-1)], axis=1)
        lr = self.conv(lr)
        lr = self.mix_lr(lr)
        left = lr[:, 0, :]
        right = lr[:, 1, :]
        ...
        for index_diag in range(start, stop):
            next_diag_pred = dlem(left, right, get_diags(torch.exp(x.squeeze()), index_diag), index_diag, x.shape[-1], transform)
